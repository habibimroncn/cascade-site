...
if ($view->override_path) {
	...
	// Add Node ID to array Keys
	
	foreach ($rows['nodes'] as &$item) {
		//$nid = $item['node']['Nid'];
		$item['node']['Body'] = check_markup($item['node']['Body']);
		$item['node']['Matrix'] = json_decode($item['node']['Matrix']);
		//$item['node-'.$nid] = $item['node'];
		//unset($item['node']);
	}
